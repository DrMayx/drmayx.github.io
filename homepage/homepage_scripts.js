...
let isEditing = false;
let registeredDomains = ["com", "org", "net", "edu", "gov",
...
function passToURL(event){
	if (event.keyCode === 13) {
		event.preventDefault();
		let searchQuery = document.getElementById("customSearchbar").value;
		let isWebsite = false;
		for(let domain of registeredDomains){
			if(searchQuery.endsWith("." + domain)){
				window.location = "https://" + searchQuery;
				isWebsite = true;
				break;
			}
		}
		if(!isWebsite){
			window.location = "https://duckduckgo.com/?t=canonical&q=" + searchQuery+ "&atb=v232-2&ia=web";
		}
	}
}
